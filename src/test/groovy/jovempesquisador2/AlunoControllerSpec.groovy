...
class AlunoControllerSpec extends Specification implements ControllerUnitTest<AlunoController>, DomainUnitTest<Aluno> {
    ...
    void "Test the index action returns the correct model"() {
        given:
        controller.alunoService = Mock(AlunoService) {
            1 * list(_) >> []
            1 * count() >> 0
        }
    ...
    void "Test the save action with an invalid instance"() {
        given:
        controller.alunoService = Mock(AlunoService) {
            1 * save(_ as Aluno) >> { Aluno aluno ->
                throw new ValidationException("Invalid instance", aluno.errors)
            }
        }
